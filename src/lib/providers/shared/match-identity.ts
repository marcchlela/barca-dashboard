...
export type MatchIdentityResolution = {
  ...
  criteria: {
    providerId: boolean;
    temporalPrecision: MatchIdentityCandidate["temporalPrecision"];
...
const COMPETITION_ALIAS_GROUPS = [
  ...
  ["uefa champions league", "champions league", "ucl", "cl"],
] as const;
...
function aliasKey(value: string, groups: readonly (readonly string[])[]) {
...
function internalHasFinalScore(internal: MatchIdentityInput) {
...
export function resolveMatchIdentity(
  ...
  const expectedProviderId = providerCode
    ? internal.providerIds[providerCode]
    : undefined;
  ...
  const homeIdentity = matchTeamIdentity(internal.homeTeam.name, candidate.homeTeam);
  const awayIdentity = matchTeamIdentity(internal.awayTeam.name, candidate.awayTeam);
  const score =
  ...
  const candidateDate =
    ...
    (candidate.temporalPrecision === "exact" && candidate.kickoff
      ...
      : null);
  const sameCalendarDate = Boolean(candidateDate && candidateDate === internalDate);
  ...
  const confidence = Math.min(
    ...
    (idMatch ? 0.3 : 0) +
      ...
      (homeIdentity.matched ? 0.15 : 0) +
      (awayIdentity.matched ? 0.15 : 0) +
      (score ? 0.2 : 0) +
      (competition ? 0.05 : 0),
  );
  ...
  const matched =
    ...
    (idMatch || finishedEvidence || upcomingEvidence);
  ...
  return {
    ...
    criteria: {
      ...
      homeTeam: homeIdentity.matched,
      homeTeamMethod: homeIdentity.method,
      awayTeam: awayIdentity.matched,
